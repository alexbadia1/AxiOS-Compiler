...
import { CHARACTER, CODE_EDITOR_TAB_LENGTH, DIGIT, END_BLOCK_COMMENT, END_OF_PROGRAM, ERROR, IDENTIFIER, INFO, INVALID_TOKEN, KEYWORD_BOOLEAN, KEYWORD_FALSE, KEYWORD_IF, KEYWORD_INT, KEYWORD_PRINT, KEYWORD_STRING, KEYWORD_TRUE, KEYWORD_WHILE, LEXER, MISSING_TOKEN, SPACE_END_OF_LINE, SPACE_SINGLE, SPACE_TAB, START_BLOCK_COMMENT, STRING_EXPRESSION_BOUNDARY, SYMBOL, SYMBOL_ASSIGNMENT_OP, SYMBOL_BOOL_OP_EQUALS, SYMBOL_BOOL_OP_NOT_EQUALS, SYMBOL_CLOSE_ARGUMENT, SYMBOL_CLOSE_BLOCK, SYMBOL_INT_OP, SYMBOL_OPEN_ARGUMENT, SYMBOL_OPEN_BLOCK, WARNING, WARNING_TOKEN } from './global'
...
export class LexerService {
  ...
  public currProgram: Program = new Program();
  public output: Array<OutputConsoleMessage> = [];

  /**
   * Source code from code editor.
   */
  public rawSourceCode: string | null = "";

  /*
     * Lexer will attempt to correct source code
     */
  public modified_source_code: string | null = null;
   ...
   */
  private _temp_token: LexicalToken | null = null;
  ...
  private _token_pool: Array<LexicalToken> = [
    ...
    // Commands
    new LexicalToken(KEYWORD_PRINT, /^(print)$/, "", -1, -1),
    new LexicalToken(KEYWORD_WHILE, /^(while)$/, "", -1, -1),
    new LexicalToken(KEYWORD_IF, /^(if)$/, "", -1, -1),

    // Types
    new LexicalToken(KEYWORD_INT, /^(int)$/, "", -1, -1),
    new LexicalToken(KEYWORD_STRING, /^(string)$/, "", -1, -1),
    new LexicalToken(KEYWORD_BOOLEAN, /^(boolean)$/, "", -1, -1),

    // Boolean keywords
    new LexicalToken(KEYWORD_TRUE, /^(true)$/, "", -1, -1),
    new LexicalToken(KEYWORD_FALSE, /^(false)$/, "", -1, -1),

    /**
     * 2. Identifiers
     */
    new LexicalToken(IDENTIFIER, /^[a-z]$/, "", -1, -1),
    ...
    // Comments
    new LexicalToken(START_BLOCK_COMMENT, /^(\/\*)$/, "", -1, -1),
    new LexicalToken(END_BLOCK_COMMENT, /^(\*\/)$/, "", -1, -1),

    // Quotation
    new LexicalToken(STRING_EXPRESSION_BOUNDARY, /^["]$/, "", -1, -1),

    // Open/Close blocks
    new LexicalToken(SYMBOL_OPEN_BLOCK, /^[\{]$/, "", -1, -1),
    new LexicalToken(SYMBOL_CLOSE_BLOCK, /^[\}]$/, "", -1, -1),

    // Open/Close arguments
    new LexicalToken(SYMBOL_OPEN_ARGUMENT, /^[\(]$/, "", -1, -1),
    new LexicalToken(SYMBOL_CLOSE_ARGUMENT, /^[\)]$/, "", -1, -1),

    // Operands
    new LexicalToken(SYMBOL_INT_OP, /^[\+]$/, "", -1, -1),
    new LexicalToken(SYMBOL_BOOL_OP_EQUALS, /^(==)$/, "", -1, -1),
    new LexicalToken(SYMBOL_BOOL_OP_NOT_EQUALS, /^(!=)$/, "", -1, -1),

    // Assignments
    new LexicalToken(SYMBOL_ASSIGNMENT_OP, /^[=]$/, "", -1, -1),

    // Digits
    new LexicalToken(DIGIT, /^[0-9]$/, "", -1, -1),

    // Whitespace
    new LexicalToken(SPACE_SINGLE, new RegExp("^ $"), "", -1, -1),
    new LexicalToken(SPACE_TAB, new RegExp("^\t$"), "", -1, -1),
    new LexicalToken(SPACE_END_OF_LINE, new RegExp("^\n$"), "", -1, -1),

    // End of Program
    new LexicalToken(END_OF_PROGRAM, /^[\$]$/, "", -1, -1),

    // Character
    new LexicalToken(CHARACTER, /^[a-z]$/, "", -1, -1),
  ];
   ...
   * 
   * @param {string} code source code from code editor.
   */
  public lex(code: string): Map<string, any> {

    // First Program
    this.currProgram = new Program(++this.numPrograms);
    this.programs.push(this.currProgram);
    this.currProgram.lexerOutput.push(new OutputConsoleMessage(LEXER, INFO, `Lexing program ${this.numPrograms}...`));

    // The current substring of the source code that will be tested against tokens.
    // current_potential_lexeme = code_substring
    let codeSubstring: string = "???";

    while (this._last_position <= code.length && this._current_position <= code.length) {

      // New substring
      codeSubstring = code.substring(this._last_position, this._current_position);
      this.currProgram.stacktrace.push(codeSubstring);

      // Still inside a comment
      if (this.isInComment) {
        // END_BLOCK_COMMENT Token is found! Skip reading the next line in comment
        if (this.check_for_end_comment(codeSubstring)) { continue; }// if

        // End Of Line is found! Update our position.
        if (codeSubstring.endsWith("\n")) {
          this._code_editor_line_number++;
          this._code_editor_line_position = 0;
        }// if
        this._current_position++;
      }// if: Still inside a comment

      // Still inside a string
      else if (this.isInString) {
        if (this.check_for_valid_character_or_space(codeSubstring)) { continue; }// if
        if (this.check_for_end_string_boundary(codeSubstring)) { continue; }// if
        if (this.check_for_end_of_line(codeSubstring)) { continue; }// if
        ...
        // Update our position
        this._code_editor_line_position += codeSubstring.length;
        this.advance_last_position();
      }// else-if: Still inside a string

      // Outside of comment or string
      else {

        // Skip testing against token pool if a sentinel value is found.
        if (this.check_for_sentinel(codeSubstring)) { continue; }// if

        // Test current substring of source code against tokens in the token pool
        let token: LexicalToken | null = this.get_token_from_token_pool(codeSubstring);

        // No token was returned. The character is probably illegal. Check to make sure.
        if (token == null) {
          this.check_illegal_characters_for_symbols(codeSubstring, code);
        }// if

        // Check if token is an open or close string expression boundary.
        else if (token.name == STRING_EXPRESSION_BOUNDARY) {
          this.check_current_string_expression_boundaries(token, codeSubstring);
        }// if

        // Make sure the symbol is not part of a larger symbol.
        else if (token.name.includes(SYMBOL)) {
          this.check_for_longer_symbol(token, codeSubstring, code);
        }// else-if

        // End of current program, check for more programs.
        else if (token.name.includes(END_OF_PROGRAM)) {
          this.check_for_more_programs(token, codeSubstring, code);
        }// else if 
        ...
        // Store ID's and Keywords in a temp variable this way the temp variable will hold the longest key in the current iteration.
        else {
          // This will end up replacing shorter tokens
          this._temp_token = token.copyWith(
            codeSubstring,
            this._code_editor_line_number,
            this._code_editor_line_position + codeSubstring.length - (codeSubstring.length - 1)
          );// copyWith
          this._current_position++;
        }// else
      }// else: Outside of comment or string
    }// while

    // Fix warnings before handing of tokens to the parser
    this.modified_source_code = this.post_check_for_warnings(code);

    // Fixing errors is even more dangerous that fixing warnings... so don't.
    this.post_check_for_errors();

    // Lex metadata: count warnings and errors
    let numWarnings: number = 0;
    let numErrors: number = 0;
    for (var prog of this.programs) {
      numWarnings += prog.warningTokenStream.length;
      numErrors += prog.errorTokenStream.length;
    }// for

    this.output.push(new OutputConsoleMessage(LEXER, INFO, `Lexer completed with ${numWarnings} warnings.`));
    this.output.push(new OutputConsoleMessage(LEXER, INFO, `Lexer completed with ${numErrors} errors.`));

    let map = new Map<string, any[]>();
    map.set('programs', this.programs);
    map.set('output', this.output);
    return map;
  }// lex
  ...
  private check_for_end_comment(new_current_potential_lexeme: string): boolean {
    ...
    if (/(\*\/$)/.test(new_current_potential_lexeme)) {
      ...
      // Generate token
      this._temp_token = new LexicalToken(
        END_BLOCK_COMMENT,
        /^(\*\/)$/,
  ...
  private check_for_valid_character_or_space(new_current_potential_lexeme: string): boolean {
    if (/^[a-z]$/.test(new_current_potential_lexeme) || RegExp("^ $").test(new_current_potential_lexeme) || RegExp("^\t$").test(new_current_potential_lexeme)) {
      ...
      // Generate token
      this._temp_token = new LexicalToken(CHARACTER, /^(\*\/)$/, new_current_potential_lexeme, this._code_editor_line_number, this._code_editor_line_position + 1);
      ...
      */
      if (this._temp_token != null) {
        ...
        if (this._temp_token.name.includes("SPACE")) {
          if (this._temp_token.lexeme != null) {
  ...
  private check_illegal_characters_for_symbols(new_current_potential_lexeme: string, source_code: string) {
    if (/(^!$|^\*$|^\/$)/.test(new_current_potential_lexeme)) {
       ...
       */
      let current_token: LexicalToken | null = this.get_token_from_token_pool(peekingChunk);
  ...
  private check_for_more_programs(new_current_token: LexicalToken, new_current_potential_lexeme: string, source_code: string) {
    ...
    this.currProgram.lexerOutput.push(new OutputConsoleMessage(LEXER, INFO, `Lexer finished lexing program ${this.numPrograms}.`));
   ...
   * change the meaning of the program slightly, hence "warning."
   * 
   * @param newnew_source_code source code from coder mirror.
   * @returns modifed source code
   */
  private post_check_for_warnings(new_source_code: string): string {
    // Missing EOP, add it
    if (this.missingEndOfProgram) {
      this.currProgram.lexerOutput.push(new OutputConsoleMessage(LEXER, WARNING, `Missing EOP. Try adding $ to line ${this._code_editor_line_number}`));
  ...
  private post_check_for_errors() {
    ...
    if (this.isInComment) {
      this.currProgram.lexerOutput.push(new OutputConsoleMessage(LEXER, WARNING, `Missing End End Comment. Try adding */ to line ${this._code_editor_line_number}`));
      this.emit_token_to_stream(new LexicalToken(MISSING_TOKEN, null, "*/", this._code_editor_line_number, -1));
      this.currProgram.isValid = false;
    }// if

    if (this.isInString) {
      this.currProgram.lexerOutput.push(new OutputConsoleMessage(LEXER, WARNING, `Missing End String Boundary Expression. Try adding \" to line ${this._code_editor_line_number}`));
      this.emit_token_to_stream(new LexicalToken(MISSING_TOKEN, null, "\"", this._code_editor_line_number, -1));
      this.currProgram.isValid = false;
    }// if
   ...
   * 
   */
  private get_token_from_token_pool(newLexeme: string): LexicalToken | null {
    let index: number = 0;
  ...
  private emit_token_to_stream(newToken: LexicalToken): void {
    if (newToken.name == WARNING_TOKEN) {
      this.currProgram.warningTokenStream.push(newToken);
    }// if

    else if (newToken.name == INVALID_TOKEN) {
      this.currProgram.isValid = false;
      this.currProgram.lexerOutput.push(new OutputConsoleMessage(
        LEXER, ERROR,
        `${this._code_editor_line_number}:${this._code_editor_line_position} Unrecognized Token: ${newToken.lexeme}`
      )
      );
      this.currProgram.errorTokenStream.push(newToken);
    }// else-if
      ...
      if (this.isInString && (newToken.name == SPACE_SINGLE || newToken.name == SPACE_TAB)) {
        this.currProgram.cleanTokenStream.push(newToken);
      }// if
    ...
    this.currProgram.debugTokenStream.push(newToken);
    this.currProgram.stacktrace.push(newToken);
